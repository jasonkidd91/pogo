...
| Directory | Purpose |
|---|---|
| `web/` | The site. Static HTML/CSS/JS, no build step. Four runtime dependencies: the Firebase SDK loaded from the CDN by `auth.js`, the ScrapedDuck event feed fetched by `events.js`, LeekDuck's promo codes page fetched by `promo-codes.js`, and ntfy.sh, which `remind.js` publishes event reminders to. |
...
# Refresh the promo-codes FALLBACK snapshot — same relationship to promo-codes.js as the line
...
- **LIVE** (current events, raid rotations, Max Battle tiers, egg pools, promo codes) — **must
  be looked up**, and generally should *not* be baked into a static file at all. The events
  page is the worked example: it fetches the feed at runtime and treats its baked copy as a
  labelled fallback. The promo codes page follows the same pattern.
...
`promo-codes.html` lists every Pokémon GO promo code LeekDuck currently shows as active, with
Copy and a direct Redeem link — same design as the events page: LIVE data, fetched on every
load, `web/promo-data.js` only the fallback shown while that request is in flight or fails.
There's no JSON feed for this the way ScrapedDuck provides one for events, so both
`web/promos.js` (the browser, via `DOMParser`) and `scripts/update_promos.py` (the generator,
via regex on raw HTML) parse LeekDuck's `.promo-card` markup independently and have to be kept
in sync by hand — see the `update-promos` skill.

**`web/promo-data.js` is loaded on every page, not just its own.** The nav's "you have an
unseen code" nudge (`Promos.unseenCount()` in `web/promos.js`, painted by `buildNav()` in
`store.js`) needs the current code list without every page paying for a live fetch, and that
snapshot is the one place in this feature where up to a day of staleness is accepted — the
dedicated page itself always re-checks live.

**The nudge is a browser convenience, not account state** — deliberately `localStorage`
(`pogo.promo.seen.v1`), not Firestore. Whether you've opened the page since a code appeared has
no business following you to another device or costing a write against the free tier; it's
exactly what "per-viewer convenience" means elsewhere in this file, just applied to a feature
instead of a UI preference.

**An expired code is disabled, never hidden.** `data-expires` carries no consistent timezone —
most are a bare local wall-clock stamp, some carry an explicit Pacific offset — and is never
normalised, same rule as an events.json stamp's trailing `Z`. `hideExpiry: true` means LeekDuck
has no confirmed deadline and the date is a placeholder that must never be shown as real.
Expiry is recomputed on every render rather than baked, so a code that lapses between snapshot
refreshes loses its Copy/Redeem controls and is struck through rather than silently staying
live.

Nothing here is tracked on a Google account — a promo code is either spent at Niantic's store
or it isn't, and this site has no way to know which — so the page carries no sign-in gate and
no `data-tracker`.
...
- **`daily-maintenance.yml`** — scheduled `0 19 * * *` UTC (03:00 Asia/Kuala_Lumpur, no DST),
  ...
  2. Runs the `update-promos` skill's `scripts/update_promos.py`; commits only if
     `web/promo-data.js` actually changed. This is what keeps the nav's "unseen promo code"
     nudge current — the nudge itself is computed client-side, this step just refreshes the
     data it reads.
  3. **Archives finished trackers.** An entry in `TRACKERS` (`web/events.js`) counts as over
  ...
  4. **Files an issue for events starting within the next 3 days** that aren't in `TRACKERS`
...
- **`claude.yml`** — the `@claude`-mention responder, triggered only by `issue_comment` and
  ...
  bodies task 4 above writes contain the literal phrase "@claude implement it" as
...
| `update-promos` | The Promo Codes page and its nav nudge — LeekDuck scraping, expiry/disable logic, the localStorage seen-tracking |
